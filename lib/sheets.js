...
function _getAuthClient() {
  ...
  // Intentar ambos formatos: con \n literal y con saltos de línea reales
  const privateKey = rawKey.includes('\\n')
    ? rawKey.replace(/\\n/g, '\n')
    : rawKey;

  console.log('[AUTH] email:', email);
  console.log('[AUTH] key starts:', privateKey.substring(0, 40));
  console.log('[AUTH] key ends:', privateKey.substring(privateKey.length - 40));
  console.log('[AUTH] spreadsheetId:', process.env.SPREADSHEET_ID);
...
async function _getSheetData(sheetName) {
  try {
    ...
    logError('_getSheetData error', { sheetName, error: e.message });
    throw e;
  }
}
